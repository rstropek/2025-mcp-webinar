"use strict";
/**
 * Chat UI: calls /api/chat and embeds the password form iframe when the assistant returns a UI resource.
 */
class LLMClient {
    constructor() {
...
async function sendMessage() {
    ...
    try {
        ...
        addMessage('assistant', response.text, response.uiResource ?? null, response.toolArgs ?? null);
    }
    catch (error) {
        addMessage('assistant', `Error: ${error instanceof Error ? error.message : String(error)}. Make sure the server is running and your API key is configured in .env`);
    }
    finally {
