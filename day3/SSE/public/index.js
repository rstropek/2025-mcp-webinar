// The page is served by the same Express app that hosts the endpoints, so the
// origin of the page is the base URL of the API (works on any port).
...
sseCustomEventsId.addEventListener("click", async () => {
  sseCustomEventsIdSpinner.hidden = false;
  ...
  eventSource.addEventListener("eom", () => {
    eventSource.close();
    sseCustomEventsIdSpinner.hidden = true;
  });
});
