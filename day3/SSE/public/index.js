...
const longRunningRegularSpinner = document.querySelector(
	"#long-running-regular-spinner",
);
const longRunningRegularOutput = document.querySelector(
	"#long-running-regular-output",
);

const longRunningStreaming = document.querySelector("#long-running-streaming");
const longRunningStreamingSpinner = document.querySelector(
	"#long-running-streaming-spinner",
);
const longRunningStreamingOutput = document.querySelector(
...
const sseCustomEventsSpinner = document.querySelector(
	"#sse-custom-events-spinner",
);
const sseCustomEventsOutput = document.querySelector(
	"#sse-custom-events-output",
);

const sseCustomEventsId = document.querySelector("#sse-custom-events-id");
const sseCustomEventsIdSpinner = document.querySelector(
	"#sse-custom-events-id-spinner",
);
const sseCustomEventsIdOutput = document.querySelector(
	"#sse-custom-events-id-output",
);
...
longRunningRegular.addEventListener("click", async () => {
	...
	try {
		...
		const response = await fetch(`${baseUrl}/long-running/regular`, {
			method: "POST",
		});
...
longRunningStreaming.addEventListener("click", async () => {
	...
	try {
		const startTime = performance.now();
		const response = await fetch(`${baseUrl}/long-running/streaming`, {
			method: "POST",
		});
...
sseDataOnly.addEventListener("click", async () => {
	...
	eventSource.onopen = () => {
		sseDataOnlyOutput.innerHTML += `SSE connection opened\n`;
	};
});
