...
router.get("/custom-events", async (request, response) => {
  ...
  response.on("close", () => {
    console.log("Client disconnected from SSE custom-events");
    cleanup();
  });
  ...
  try {
    ...
    console.error("Error in SSE custom-events:", error);
    cleanup();
  }
});
...
router.get("/custom-events-with-id", async (request, response) => {
  ...
  response.on("close", () => {
    console.log("Client disconnected from SSE custom-events-with-id");
    cleanup();
  });
  ...
  try {
    ...
    if (lastEventId) {
      ...
      if (Number.isNaN(parsed) || parsed < 0) {
        console.warn(`Invalid Last-Event-Id header: ${lastEventId}`);
        response.write(`: Invalid Last-Event-Id, starting from 0\n\n`);
      } else {
        i = parsed + 1;
        console.log(`Resuming from event ID ${i}`);
      }
    }
    for (; i < 10; i++) {
      if (aborted) break;
    ...
    console.error("Error in SSE custom-events-with-id:", error);
    cleanup();
  }
});
