...
# upgrades the response from a single JSON object to an SSE stream. `curl -N`
# disables curl's output buffering, so the three progress events appear one per
# second and the result arrives last — the point of Streamable HTTP, live in the
# terminal.
#
# Usage: ./curl.sh            (defaults to port 3000)
#        PORT=3100 ./curl.sh
...
curl -N -s -i -X POST "http://127.0.0.1:${PORT}/mcp" \
