...
//
// Nothing about the tools is known at compile time. The bot connects to the
// Sample 2 MCP server over Streamable HTTP, asks it `tools/list` at runtime,
// converts whatever comes back into Responses-API function tools and forwards
// every tool call straight back to the server via `tools/call`.
//
// That is the whole point of MCP: the model's capabilities are discovered, not
// compiled in. Add a tool to `server-sdk-streamable.ts`, restart the server —
...
const openai = new OpenAI({
  ...
  defaultHeaders: {
    ...
    "X-OpenRouter-Title": "MCP Pony Chat (Streamable HTTP)",
  },
});

// The same cheap model as in the pi.dev tests.
const MODEL = "z-ai/glm-5.3-flash";

// Unlike a stdio server, an HTTP server is NOT spawned by its client: start
// `npm run start:sdk` in another terminal first. This also means several chat
// clients can share one server process.
// `versionNegotiation.mode: "auto"` probes with `server/discover` first and falls
// back to the legacy 2025-era `initialize` handshake (see client-streamable.ts).
...
try {
  ...
  // `close()` tears down the transport and aborts any in-flight request. The
  // server keeps running — it is a separate process.
  await mcp.close();
}
