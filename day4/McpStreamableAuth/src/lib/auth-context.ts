 ...
 * Useful for logging, debugging, and session-specific caching.
 */
...
/**
 * Returns the OAuth 2.0 scopes on the current request's token.
 *
 * Scalekit (like most issuers) emits the standard `scope` claim: a
 * single space-separated string (RFC 6749 §3.3). Some issuers also/instead
 * emit `scopes` as an array — we accept either.
 */
